...
const ThreadHistory = React.forwardRef<HTMLDivElement, ThreadHistoryProps>(
  (
    { className, onThreadChange, defaultCollapsed = true, position = "left", children, ...props },
    ref,
  ) => {
    ...
    return (
      <ThreadHistoryContext.Provider value={contextValue as ThreadHistoryContextValue}>
        <div
          ref={ref}
          className={cn(
            "bg-card/50 backdrop-blur-sm h-full transition-all duration-300 flex-none border-border/50",
          ...
          {...props}
        >
          <div className={cn("flex flex-col h-full", isCollapsed ? "py-4 px-2" : "p-4")}>
            {children}
          </div>
        </div>
    ...
    >
      ...
      <span
        className={cn(
          ...
          isCollapsed
            ? "opacity-0 max-w-0 overflow-hidden pointer-events-none absolute left-14"
 ...
 */
const ThreadHistoryHomeButton = React.forwardRef<
  HTMLAnchorElement,
...
ThreadHistoryHomeButton.displayName = "ThreadHistory.HomeButton";
